...
class _SERIAL_PORT_CANCELIO
{
	public:
		_SERIAL_PORT_CANCELIO();
		~_SERIAL_PORT_CANCELIO();
...
_SERIAL_PORT_CANCELIO::_SERIAL_PORT_CANCELIO ()
{
...
std::vector<CString> CSerialPort::EnumSerialPorts(void)
{
	...
	// COM ports can be numbered from 1 to 255,
	// loop through all possibilities and add the ones we find
...
// Read dwCount synchronously
// Returns on:
// * requested number of bytes read
// * A write operation completes on the write end of the pipe
// * Asynchronous mode is used (not this funciton)
// * An error occurs
...
// Read COM port synchronously until terminator
// Empty string in case terminator was not found
CString CSerialPort::ReadUntil(char terminator)
{
	ASSERT( IsOpen() );
	ASSERT(!m_bOverlapped);
	...
	DWORD dwBytesRead = 0;
	while( ReadFile(m_hComm, &cBuf, 1, &dwBytesRead, NULL) && dwBytesRead)
	{
		if (cBuf == terminator) return strResult;

		strResult += cBuf;
	}

	// if no terminator marker has been received return empty string
	return strEmpty;
}


// Read dwCount asynchronously
// Read fails with ERROR_IO_PENDING error to prevent another read
// To detect the actual completion of the event call WaitForSingleObject
...
// Write synchronously dwCount of bytes
// Function returns if:
// * The requested amount of bytes is written
// * A read operation releases buffer space on the reading end of the pipe
// * Asynchronous call (not this function)
// * An error occurs
...
// Write dwCount asynchronously
// Write fails with ERROR_IO_PENDING error
// To detect the actual completion of the event call WaitForSingleObject
...
// Retrieve the results of an overlapped operation
// If bWait is TRUE and the status is STATUS_PENDING the function does not return
// until the opertion has been completed
...
// Cancel all pending asynchronous IO in the calling thread
void CSerialPort::CancelIo()
{
	ASSERT( IsOpen() );

	if(_SerialPortCancelIO.m_lpfnCancelIo == NULL)
	{
	...
	if( !::_SerialPortCancelIO.m_lpfnCancelIo(m_hComm) )
	{
...
void CSerialPort::WriteEx(const void *lpBuf, DWORD dwCount)
{
	...
	// WriteFileEx ignores hEvent, an application can use it for its own purposes
...
void CSerialPort::ReadEx(void *lpBuf, DWORD dwCount)
{
	...
	// ReadFileEx ignores hEvent, an application can use it for its own purposes
	pOverlapped->hEvent = (HANDLE) this;
...
// Transmit a specified character ahead of any pending data
// Can be useful for sending an interrupt character CTRL+C
...
// Suspend character transmission on the port and place the line in break state
// until ClearBreak
...
void CSerialPort::ClearDTR() { Escape(CLRDTR); }
void CSerialPort::ClearRTS() { Escape(CLRRTS); }
void CSerialPort::SetDTR()   { Escape(SETDTR); }
void CSerialPort::SetRTS()   { Escape(SETRTS); }
void CSerialPort::SetXOFF()  { Escape(SETXOFF);}
void CSerialPort::SetXON()   { Escape(SETXON); }
...
// Get the current status of the modem control register:
// CTS (clear to send), DTS (data set ready), RINGON (ring indicator is on), RLSD (receive line signal detect)
...
// Specify a set of events to be monitored by WaitEvent
...
// Get a set of events to be monitored by WaitEvent
...
// Flush the buffer causing all buffered data to be written to a file
// Blocking function
...
// Discards all characters from the output or input buffer of a cpecified communication resource
...
void CSerialPort::TerminateOutstandingWrites() { Purge(PURGE_TXABORT); }
void CSerialPort::TerminateOutstandingReads()  { Purge(PURGE_RXABORT); }
void CSerialPort::ClearWriteBuffer()           { Purge(PURGE_TXCLEAR); }
void CSerialPort::ClearReadBuffer()            { Purge(PURGE_RXCLEAR); }

// After CreateFile, but before any IO on the port it is possible to set the sizes of internal input and
// output buffer in bytes. Default parameters are used otherwise
...
// The timeouts determine the behavious of ReadFile, WriteFile, ReadFileEx and WriteFileEx
// The mentioned funcs return successful state
...
// Read operation is to return immediately with the bytes that have been already received,
// even if no bytes have been received
// Do not use time out for write operations
...
// Do not use time out for write operations
// Nonoverlapped opeartions will block untill all the requested bytes are sent out
...
// Monitor a set of events for a resource
// Blocks until one of the specified events or an error occurs
// Non-overlapped version
// Hint: Use SetMask and GetMask to set and query the current event mask
void CSerialPort::WaitEvent(DWORD &dwMask)
{
	ASSERT( IsOpen() );
	ASSERT(!m_bOverlapped);

	if( !WaitCommEvent(m_hComm, /*out*/&dwMask, NULL) )
	{
...
// Monitor a set of events for a resource as an overlapped operation.
// overlapped must contain a handle to a manual-reset event object (created by CreateEvent function)
